...
var TimeKnots = {
  draw: function(id, events, options){
    ...
    var svg = d3.select(id).append('svg').attr("class", "timeline-svg").attr("width", cfg.width).attr("height", cfg.height + 360);
    ...
    .attr("y1", function(d){
                      ...
                      console.log("Datum: ", (cfg.dateDimension)?new Date(d.date).getTime():d.value);
                      console.log("Ret", ret);
    ...
    .attr("y2", function(d){
                      ...
                      if (linePrevious.y1 != null){
                        console.log(linePrevious.y1 + DISP_Y);
                        return linePrevious.y1 + DISP_Y
                      }
                      if(cfg.horizontalLayout){
                        console.log(Math.floor(cfg.height/2) + DISP_Y);
                        return Math.floor(cfg.height/2) + DISP_Y
                      }
                      var datum = (cfg.dateDimension)?new Date(d.date).getTime():d.value;
                      console.log(Math.floor(step*(datum - minValue)) + DISP_Y);
    ...
    svg.selectAll("line_2")
      ...
      .attr("x2", function(d) {
        ...
        if(cfg.horizontalLayout){
          var datum = (cfg.dateDimension)?new Date(d.date).getTime():d.value;
          var x=  Math.floor(step*(datum - minValue) + margin);
          return x + (scalar * SEPARATION_AMOUNT);
        }
        const val = cfg.displacement == 0 ? cfg.radius*2 : cfg.width*cfg.displacement
        return Math.floor(val) + (scalar * SEPARATION_AMOUNT)/2;
      })
      .attr("y1", function(d) {
        if(cfg.horizontalLayout){
          return Math.floor(cfg.height/2) + DISP_Y
      ...
      svg.selectAll("line_3")
        ...
        .attr("x2", function(d) {
          ...
          const val = cfg.displacement == 0 ? cfg.radius*2 : cfg.width*cfg.displacement
          return Math.floor(val) + (scalar * SEPARATION_AMOUNT);
        })
        .attr("y1", function(d) {
          var bias = 0
          if(countY1 > 0) {
    ...
    if(cfg.showLabels != false){
      ...
      console.log(cfg.globalMax, cfg.expectedGrad.timestamp);
      if (cfg.globalMax != cfg.expectedGrad.timestamp) {
        svg.append("text")
          .text("Expected Graduation").style("font-size", "16px")
          .attr("x", 0)
          .attr("y", function(d){
            var ret;
            var datum = new Date(cfg.expectedGrad.timestamp).getTime();
    ...
    svg.append("text")
       .html("Student ID is: " + events[0].id).style("font-size", "20px")
       .attr("x", "50%")
