...
d3.parsets = function() {
    ...
    selection.each(function(data, i) {
      ...
      function updateDimensions() {
        ...
        dimension
            .call(d3.drag()
              .subject(identity)
              .on("start", function(d) { //originally dragstart
                dragging = true;
                d.y0 = d.y;
              })
              .on("drag", function(d) {
              ...
              .on("end", function(d) {  //originally dragend
                dragging = false;
                unhighlight();
                var y0 = 45,
  ...
  function ribbonPath(d) {
    var s = d.source,
        t = d.target;
    return ribbonPathString(s.node.x + s.x0, s.dimension.y, s.dx, t.node.x + t.x0, t.dimension.y, t.dx, tension0);
  }
